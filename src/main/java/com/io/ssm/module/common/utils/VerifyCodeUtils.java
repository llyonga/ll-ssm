...
public class VerifyCodeUtils {
    ...
    //使用到Algerian字体，系统里没有的话需要安装字体，字体只显示大写，去掉了1,0,l,o几个容易混淆的字符
    public static final String VERIFY_CODES = "abcdefghijkmnopqrstuvwxyz23456789ABCDEFGHJKLMNPQRSTUVWXYZ";
    //验证码的Key
    ...
    public static String createVerifyCode(int length) {
        ...
        StringBuilder verify_code = new StringBuilder(length);
        for (int i = 1; i <= length; i++) {
            int index = new Random().nextInt(chars.length);
            verify_code.append(chars[index]);
        }
        return verify_code.toString();
    }
     ...
     * @return
     */
    public static BufferedImage createVerifyCodeImage(int width, int height, String verifyCode) {
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        Graphics2D gd = image.createGraphics();
        gd.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        gd.setColor(Color.WHITE);
        gd.fillRect(0, 0, width, height);
        gd.setFont(new Font("微软雅黑", Font.ITALIC, 80));
        gd.dispose();
        return image;
    }
    ...
    public static void clearVerifyCode() {
        SecurityUtils.getSubject().getSession().removeAttribute(VERIFY_CODE);
    }
